...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B: int, H: int, W: int):
        ...
        x_embed = torch.arange(1, W + 1, dtype=torch.float32, device=device).repeat(
            1, H, 1
        )
...
class FAN(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            ...
            # TODO: Check Issue https://github.com/NVlabs/FAN/issues/13 for official fix
...
def checkpoint_filter_fn(state_dict, model):
    ...
    if (
        "cls_attn_blocks.0.attn.qkv.weight" in state_dict
        and "cls_attn_blocks.0.attn.q.weight" in model.state_dict()
    ):
        ...
        for i in range(num_ca_blocks):
            ...
            for j, subscript in enumerate("qkv"):
                state_dict[f"cls_attn_blocks.{i}.attn.{subscript}.weight"] = qkv_weight[
                    j
                ]
            ...
            if qkv_bias is not None:
                ...
                for j, subscript in enumerate("qkv"):
                    state_dict[f"cls_attn_blocks.{i}.attn.{subscript}.bias"] = qkv_bias[
                        j
                    ]
    return state_dict
...
#TODO: Fix Register CFG and model calls to only return kwargs based on only_config=True
def register_cfg(fn):
    model_name = fn.__name__
    cfg = fn()[1]
    _all_configs[model_name] = cfg
...
def fan_tiny_12_p16_224(pretrained=False, bn_tf=False, **kwargs):
    ...
    model = _create_fan("fan_tiny_12_p16_224", pretrained=pretrained, **model_kwargs)
...
def fan_small_12_p16_224_se_attn(pretrained=False, **kwargs):
    ...
    model = _create_fan("fan_small_12_p16_224", pretrained=pretrained, **model_kwargs)
    return model, model_kwargs
...
def fan_base_18_p16_224(pretrained=False, **kwargs):
    ...
    model = _create_fan("fan_base_18_p16_224", pretrained=pretrained, **model_kwargs)
    return model, model_kwargs
...
def fan_large_24_p16_224(pretrained=False, **kwargs):
    ...
    model = _create_fan("fan_large_24_p16_224", pretrained=pretrained, **model_kwargs)
    return model, model_kwargs
...
def fan_tiny_8_p4_hybrid(pretrained=False, **kwargs):
    ...
    bbone_args = dict(depths=[3, 3], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(
        pretrained=False, pretrained_strict=False, **bbone_args
    )
    bbone_args["backbone"] = "hybrid"
    ...
    model_kwargs.update(bbone_args)
    return model, model_kwargs
...
def fan_large_16_p4_hybrid(pretrained=False, **kwargs):
    depth = 22
    sr_ratio = [1] * (depth // 2) + [1] * (depth // 2)
    bbone_args = dict(depths=[3, 5], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(
        pretrained=False, pretrained_strict=False, **bbone_args
    )
    bbone_args["backbone"] = "hybrid"
...
def fan_Xlarge_16_p4_hybrid(pretrained=False, **kwargs):
    ...
    sr_ratio = [1] * (depth // 2) + [1] * (depth // 2 + 1)
    bbone_args = dict(depths=[3, 7], dims=[128, 256, 512, 1024], use_head=False)
    backbone = _create_hybrid_backbone(
        pretrained=False, pretrained_strict=False, **bbone_args
    )
    bbone_args["backbone"] = "hybrid"
...
def fan_swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    ...
    return _create_fan_swin_transformer(
